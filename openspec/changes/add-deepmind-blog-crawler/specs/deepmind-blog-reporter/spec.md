...
### Requirement: Generate daily news section for DeepMind blog
The system SHALL generate a formatted section in the daily news report displaying DeepMind's latest blog posts.

#### Scenario: Create report section
- **WHEN** generating the daily news report
- **THEN** it SHALL create a section titled "DeepMind Blog"
- **THEN** it SHALL retrieve blog posts from the `deepmind_blog` MongoDB collection
...
- **THEN** it SHALL use the `deepmind_blog` collection name
...
- **THEN** the DeepMind Blog reporter SHALL be called
