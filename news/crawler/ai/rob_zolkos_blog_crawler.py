...
ROB_ZOLKOS_ATOM_URL = "https://www.zolkos.com/feed.xml"
COLLECTION_NAME = "rob_zolkos_blog"
...
class RobZolkosBlogParser(WebParser):
    def parse(self, resp_text: str) -> List[dict]:
        ...
        logger.info(f"{len(blog_list)} rob zolkos blog posts parsed")
        return blog_list
    ...
    def _parse_item(self, entry) -> dict:
        try:
            ...
            logger.warning(f"Failed to parse rob zolkos blog item: {e}")
            return None


def crawl():
    parser = RobZolkosBlogParser()
    web_crawler.crawl(
        parser, ROB_ZOLKOS_ATOM_URL, COLLECTION_NAME, proxies=config["proxies"]
    )
